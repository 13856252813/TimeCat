...
import com.cary.activity.timecat.fragment.index.fulldress.FullDressOrderActivity;
 ...
 * 确认订单 页面
 */
public class FullDressDetialOrderActivity extends BaseActivity {

    private static final String TAG = FullDressOrderActivity.class.getSimpleName();
    ...
    private FullDressDetialResult.Data mFdDetial;//服装详情的model
...
//        setContentView(R.layout.activity_full_dress_detial_order);
        ...
        mFdDetial = (FullDressDetialResult.Data) getIntent().getSerializableExtra("fulldressdetial");
        ...
        if (1 == photoFlag) {
            ...
            //租借
            tvDetialOrderClothAffect.setText("服装租借");
            ClothMoney = mFdDetial.getAmount() + mFdDetial.getPaiAmount();
            tvDetialOrderClothPrice.setText("租金¥" + mFdDetial.getAmount() + "押金¥" + mFdDetial.getPaiAmount());
        } else if (3 == photoFlag) {
            //拍照
            tvDetialOrderClothAffect.setText("服装拍照");
            ClothMoney = mFdDetial.getAmount() + mFdDetial.getPaiAmount();
            tvDetialOrderClothPrice.setText("租金¥" + mFdDetial.getAmount() + "押金¥" + mFdDetial.getPaiAmount());
        } else if (4 == photoFlag) {
            //积分商城
            tvDetialOrderClothPrice.setText("积分" + " " + mIntegral.getScore());
            tvDetialOrderClothAffect.setText("积分兑换");
            ClothMoney = mIntegral.getScore();//价格是积分
        }
        String imgUrl = "";
        //根据接受的值 来展示是积分 还是购买
        if (mFdDetial != null) {
            Title = mFdDetial.getTitle();
            StoreName = mFdDetial.getStore().getStoreName() + " >";
        ...
        public void onResponse(Call<NewReceivedResult> call, Response<NewReceivedResult> response) {
            if (response.isSuccessful()) {
                ...
                if ("00".equals(defaultAddRes.getCode())) {
                    ...
                    ToastUtil.showShort(FullDressDetialOrderActivity.this, defaultAddRes.getMsg());
                }
            } else {
        ...
        public void onResponse(Call<ModelBeanData> call, Response<ModelBeanData> response) {
            if (response.isSuccessful()) {
                ...
                if ("00".equals(beanData.getCode())) {
                    ...
                    ToastUtil.showShort(FullDressDetialOrderActivity.this, beanData.getMsg());
                }
            } else {
                Log.e(TAG, "+++" + response.message());
            }
