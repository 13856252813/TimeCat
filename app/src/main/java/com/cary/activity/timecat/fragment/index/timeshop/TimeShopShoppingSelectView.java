...
import com.cary.activity.timecat.fragment.index.fulldress.selection.FlowLayout;
 ...
 * 描述:     TODO 商品规格选择View
 */
public class TimeShopShoppingSelectView extends LinearLayout {
    /**
     * 数据源
     */
//    private List<FullDressColothNorm.Data> list;
    private TimeShopDetialResult list;
    /**
     * 上下文
     */
    ...
    public TimeShopShoppingSelectView(Context context) {
        super(context);
        initView(context);
    }

    public TimeShopShoppingSelectView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView(context);
    }
    ...
    public void getView() {
        if (list.getData().getNorms().size() < 0) {
            return;
        }
//        for (FullDressColothNorm.Data attr : list) {
        for (TimeShopDetialResult.DataBean.NormsBean attr : list.getData().getNorms()) {
            //设置规格分类的标题
...
//            for (int i = 0; i < attr.getValue_list().size(); i++) {
            for (int i = 0; i < attr.getItems().size(); i++) {
//                GoodsBean.SkuInfoBean.ValueListBean smallAttr = attr.getValue_list().get(i);
                TimeShopDetialResult.DataBean.NormsBean.ItemsBean smallAttr = attr.getItems().get(i);
                //属性按钮
                ...
                button.setTextColor(getResources().getColor(R.color.radiobutton_textcolor));
//                button.setText(smallAttr.getValue_name());//设置规格内容
//                button.setId(smallAttr.getValue_id());//设置规格ID
                button.setText(smallAttr.getItem());//设置规格内容
                button.setId(smallAttr.getNormId());//设置规格ID
    ...
    public void setData(TimeShopDetialResult data) {
        list = data;
        getView();
    }
