...
public interface LoginService {
    ...
    @FormUrlEncoded
    @POST("/api/WebUser/Login/QQ")
    Call<LoginCommitResult> createCommitQQ(
    ...
    @FormUrlEncoded
    @POST("/api/WebUser/Login/Weixin")
    Call<LoginCommitResult> createCommitWeChat(
            @Field("uid") String openid,
            @Field("deviceType") String deviceType,//设备类型(android-安卓,ios-苹果)
            @Field("type") String type,//类型(0个人,1老师,2管理人员)
            @Field("deviceToken") String deviceToken//友盟token device
    ...
    @FormUrlEncoded
    @POST("/api/WebUser/Login/Weibo")
    Call<LoginCommitResult> createCommitSina(
            @Field("uid") String openid,
            @Field("deviceType") String deviceType,//设备类型(android-安卓,ios-苹果)
            @Field("type") String type,//类型(0个人,1老师,2管理人员)
            @Field("deviceToken") String deviceToken//友盟token device
