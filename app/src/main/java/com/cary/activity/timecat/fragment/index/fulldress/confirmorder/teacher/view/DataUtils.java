package com.cary.activity.timecat.fragment.index.fulldress.confirmorder.teacher.view;
...
public class DataUtils {
	public static SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy年MM月");
	...
	public static ArrayList<DateEntity> getMonth(String date){
		...
		Calendar cal =Calendar.getInstance();
		try {
			cal.setTime( new SimpleDateFormat("yyyy年MM月").parse(date));
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		for (int i = 1; i <=max; i++) {
			DateEntity entity = new DateEntity();
			entity.date = getValue(cal.get(cal.YEAR))+"年"+getValue(cal.get(cal.MONTH)+1)+"月";//+getValue(cal.get(cal.DATE));
			entity.million = cal.getTimeInMillis() ;
			entity.weekNum = cal.get(Calendar.DAY_OF_WEEK);
			entity.day = getValue(cal.get(cal.DATE));
			entity.weekName = getWeekName(entity.weekNum);
