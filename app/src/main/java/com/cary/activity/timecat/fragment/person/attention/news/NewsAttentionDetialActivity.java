...
public class NewsAttentionDetialActivity extends BaseActivity {

    private static final String TAG = NewsAttentionDetialActivity.class.getSimpleName();
    ...
    private NewsAttentionDetialResult mNewsRes;
    ...
    public int getLayout() {
        return R.layout.activity_news_attention_detial;
    }

    //    动态咨询 加载数据
    private Callback<NewsAttentionDetialResult> callbackNews = new Callback<NewsAttentionDetialResult>() {
        @Override
        public void onResponse(Call<NewsAttentionDetialResult> call, Response<NewsAttentionDetialResult> response) {
            if (response.isSuccessful()) {
                ...
                if ("00".equals(mNewsRes.getCode())) {
                    if (mNewsRes != null) {
                        titleStr = mNewsRes.getData().getInformation().getTitle();
                        createTimeStr = mNewsRes.getData().getInformation().getCreateTime();
                        contentStr = mNewsRes.getData().getInformation().getContent();
                    ...
                    ToastUtil.showShort(NewsAttentionDetialActivity.this, mNewsRes.getMsg());
                }
            } else {
        ...
        @Override
        public void onFailure(Call<NewsAttentionDetialResult> call, Throwable t) {
            Log.e(TAG, "***" + t.getMessage());
        }
    };

    private void createSingleNews(int id) {
        Call<NewsAttentionDetialResult> call = mNewsApi.getService().createCommitID(token, id);
