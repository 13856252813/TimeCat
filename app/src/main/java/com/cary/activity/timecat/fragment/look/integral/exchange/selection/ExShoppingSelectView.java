 ...
 * 描述:     TODO 商品规格选择View
 */
public class ExShoppingSelectView extends LinearLayout {
    /**
     * 数据源
     */
//    private List<FullDressColothNorm.Data> list;
    private IntegralDetialResult list;
    /**
     * 上下文
     */
    ...
    public ExShoppingSelectView(Context context) {
        super(context);
        initView(context);
    }

    public ExShoppingSelectView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView(context);
    }
...
//        for (FullDressColothNorm.Data attr : list) {
        for (IntegralDetialResult.Norms attr : list.getData().getNorms()) {
            //设置规格分类的标题
...
//                GoodsBean.SkuInfoBean.ValueListBean smallAttr = attr.getValue_list().get(i);
                IntegralDetialResult.Items smallAttr = attr.getItems().get(i);
                //属性按钮
...
//                button.setId(smallAttr.getValue_id());//设置规格ID
                button.setText(smallAttr.getName());//设置规格内容
    ...
    public void setData(IntegralDetialResult data) {
        list = data;
        getView();
    }
