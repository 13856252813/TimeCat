...
public class RecyclerViewSStoreAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private Context mContext;
    //泛型是RecyclerView所需的Bean类
    private List<SelectStoreCommitResult.Data> mDateBeen;
    ...
    public RecyclerViewSStoreAdapter(Context  context) {
        mContext = context;
    }
    @Override
    ...
    public RecyclerView.ViewHolder onCreateItemViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.activity_select_store_item_layout, parent, false);
        return new ListViewHolder(view, mClickListener,mViewClickListener);
    }
    ...
    public SelectStoreCommitResult.Data getItem(int position) {
    ...
    public void reSetData(List<SelectStoreCommitResult.Data> list) {
        this.mDateBeen = list;
        notifyDataSetChanged();
    }
...
//    }
    ...
    public class ListViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        private OnItemViewClickListener mViewListener;
        public ListViewHolder(View itemView, OnItemClickListener listener,OnItemViewClickListener mViewListener) {
            super(itemView);
            this.mListener = listener;
            this.mViewListener = mViewListener;
            mImageView = (ImageView) itemView.findViewById(R.id.activity_select_store_list_item_img);
            mTextView = (TextView) itemView.findViewById(R.id.activity_select_store_list_item_title);
            mTvAddress = (TextView) itemView.findViewById(R.id.activity_select_store_list_item_address);
            mTvCustomer = (TextView) itemView.findViewById(R.id.activity_select_store_list_item_customer);
            mRatingBar = (RatingBar) itemView.findViewById(R.id.activity_select_store_list_item_ratingbar);
            mTvSelect = itemView.findViewById(R.id.tv_select_store);
            itemView.setOnClickListener(this);
            mTvSelect.setOnClickListener(this);
        }

        public void setData(SelectStoreCommitResult.Data data) {
            //给imageView设置图片数据
...
//                    .transform(new GlideCircleTransform(mContext, 2, mContext.getResources().getColor(R.color.black)));
            ...
            Glide.with(mContext).load(imageUrl).apply(options2).into(mImageView);
            //给TextView设置文本数据
            mTextView.setText(data.getStoreName());
            mTvAddress.setText(data.getProvince()+data.getCity()+data.getDetail());
            mTvCustomer.setText("服务顾客："+data.getServiceCount()+"人次");
            mRatingBar.setRating(data.getStarCount());
        }
        ...
        public void onClick(View v) {
             if(v==mTvSelect)
            {
              mViewClickListener.OnClickView(v,getPosition());
            }else{
