...
public class DataUtils {
     ...
     * @return
     */
    public static ArrayList<DateEntity> getMonth(String date, List<ScheduleResult.Data> schedule) {
        ArrayList<DateEntity> result = new ArrayList<>();
        Calendar cal = Calendar.getInstance();
        try {
        ...
        try {
            for (int i = 1; i <= max; i++) {
                DateEntity entity = new DateEntity();
                String dateStr = getValue(cal.get(cal.YEAR)) + "-" + getValue(cal.get(cal.MONTH) + 1) + "-" + getValue(cal.get(cal.DATE));
                entity.date = dateStr;
                entity.million = cal.getTimeInMillis();
                entity.weekNum = cal.get(Calendar.DAY_OF_WEEK);
                entity.day = getValue(cal.get(cal.DATE));
                entity.weekName = getWeekName(entity.weekNum);
                entity.isToday = isToday(entity.date);
//			    entity.luna = getLuna(date);
//                entity.luna = schedule.get(0).getNickname();

                for (int i1 = 0; i1 < schedule.size(); i1++) {
//                    Log.v(TAG, "85  schedule.get(i1).getScheduleDate():" + schedule.get(i1).getScheduleDate());
                    SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                    Date dates = format.parse(schedule.get(i1).getScheduleDate());
                    // 获取日期实例
                    ...
                    if (month < 10) {
                        scheduleData = scheduleData + "0" + month;
                    } else {
                        scheduleData = scheduleData + month;
                    }
                    if (day < 10) {
                        scheduleData = scheduleData + "-0" + day;
                    } else {
                        scheduleData = scheduleData + "-" + day;
                    }
//                    Log.v(TAG, "109 --scheduleData:" + scheduleData + "== dteStr:" + dateStr);
                    if (scheduleData.equals(dateStr)) {
//                        Log.v(TAG, "111  schedule.get(i1).getNickname():" + schedule.get(i1).getNickname());
                        entity.luna = schedule.get(i1).getNickname();
                        break;
                    } else {
//				        Log.v(TAG,"114  dateStr:"+dateStr);
                        entity.luna = "";
                    }
                }
                cal.add(Calendar.DATE, 1);
//                Log.v(TAG, "111 luna:" + entity.luna);
                result.add(entity);
            }
        } catch (Exception e) {
            e.printStackTrace();
