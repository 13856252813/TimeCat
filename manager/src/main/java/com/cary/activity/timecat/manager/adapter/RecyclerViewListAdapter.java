...
public class RecyclerViewListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private List<GrabResult.Data> mList;
    ...
    public RecyclerView.ViewHolder onCreateItemViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.fragment_content_item, parent, false);
    ...
    public GrabResult.Data getItem(int position) {
        return mList.get(position);
    }

    public void addAll(List<GrabResult.Data> list) {
    ...
    public void reSetData(List<GrabResult.Data> list) {
        this.mList = list;
        notifyDataSetChanged();
    }
...
//    }
    ...
    public class ListViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        private TextView mTVGrabCreateTime;
        private TextView mTVGrabContent;
        private TextView mTVGrabAddress;
        private TextView mTVGrabMoney;
        private TextView mTVGrabitem;
        ...
        public ListViewHolder(View itemView, OnItemClickListener listener) {
            super(itemView);
            mListener = listener;
            mImageView = (ImageView) itemView.findViewById(R.id.iv_grab_typeimg);
            mTVUsername = (TextView) itemView.findViewById(R.id.tv_grab_username);
            mTVGrabCreateTime = (TextView) itemView.findViewById(R.id.tv_grab_createtime);
            mTVGrabContent = (TextView) itemView.findViewById(R.id.tv_grab_content);
            mTVGrabAddress = (TextView) itemView.findViewById(R.id.tv_grab_address);
            mTVGrabMoney = (TextView) itemView.findViewById(R.id.tv_grab_money);
            mTVGrabitem = (TextView) itemView.findViewById(R.id.tv_grab_item);
            itemView.setOnClickListener(this);
            mTVGrabitem.setOnClickListener(this);
        }

        public void setData(GrabResult.Data data) {
            //给imageView设置图片数据
...
//                    .transform(new GlideCircleTransform(mContext, 2, mContext.getResources().getColor(R.color.black)));
            String imageUrl = "https://ss2.bdstatic.com/70cFvnSh_Q1YnxGkpoWK1HF6hhy/it/u=1149604226,3297619402&fm=200&gp=0.jpg";
//            HttpUrlClient.ALIYUNPHOTOBASEURL +data.getImgurl();
            Glide.with(mContext).load(imageUrl).apply(options2).into(mImageView);
//            给TextView设置文本数据
            mTVUsername.setText(data.getUid() + "用户id");
            mTVGrabMoney.setText("¥" + data.getAmount());
            mTVGrabAddress.setText(data.getServiceCity());
            mTVGrabCreateTime.setText(data.getServiceTime());
            mTVGrabContent.setText(data.getContent());
        }
