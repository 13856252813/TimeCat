...
public class TaskDetialActivity extends AppCompatActivity {
...
//        getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_NAVIGATION);
        ...
        titleBack.setImageDrawable(getResources().getDrawable(R.mipmap.left_arrow_w));

        taskApi = GrabApi.getApi();
        ...
        public void onResponse(Call<TaskDetialResult> call, Response<TaskDetialResult> response) {
            if (response.isSuccessful()) {
                ...
                if ("00".equals(taskRes.getCode())) {
                    ...
                    teacherId = taskRes.getData().getTeacherId();
                    if (teacherId != 0) {
                        btnTaskDetialConfirmTeacher.setClickable(true);
                        btnTaskDetialConfirmTeacher.setText("抢单");
                        btnTaskDetialConfirmTeacher.setBackgroundColor(getResources().getColor(R.color.login_color_btn));
                    } else {
                        btnTaskDetialConfirmTeacher.setClickable(false);
                        btnTaskDetialConfirmTeacher.setText("已结束");
                        btnTaskDetialConfirmTeacher.setBackgroundColor(getResources().getColor(R.color.gray));
                    }
                } else {
